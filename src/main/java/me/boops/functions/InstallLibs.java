...
public class InstallLibs {
    ...
    public InstallLibs() {
        ...
        for (int i = 0; i < this.libURLS.size(); i++) {
            String distDir = new ReplaceChars().replace(Main.home_dir + "libraries" + File.separator + libURLS.get(i).substring(libURLS.get(i).indexOf(".net/") + 5, libURLS.get(i).lastIndexOf("/") + 1), "/", File.separator);
        ...
        for (int i = 0; i < libURLS.size(); i++) {
            if (!libURLS.get(i).contains("natives")) {
                InstallLibs.libs.add(new ReplaceChars().replace(Main.home_dir + "libraries" + File.separator + libURLS.get(i).substring(libURLS.get(i).indexOf(".net/") + 5, libURLS.get(i).length()), "/", File.separator));
            }
        }
    }
