...
public class ForgeHandler {
    ...
    public ForgeHandler(String[] launcher_args, String profile_forge_version, String[] meta) {
        ...
        for (int i = 0; i < launcher_args.length; i++) {
            ...
            if (launcher_args[i].equalsIgnoreCase("--with-forge")) {
                runList(launcher_args[i + 1], meta);
            }

            if (launcher_args[i].equalsIgnoreCase("--profile") && !profile_forge_version.isEmpty()) {
                runList(profile_forge_version, meta);
            }
        }
    }

    private void runList(String version_id, String[] meta) {
        ...
        String[] file_info = new ForgeFileName().getInfo(version_id, meta[0]);
        ...
        if (ForgeHandler.forge_version > 15.2) {
            ...
            ForgeHandler.libs = new FetchForgeLibs().fetch(new JSONObject(meta[1]));
        ...
        if (ForgeHandler.forge_version <= 15.2 && ForgeHandler.forge_version >= 12.5) {
            new Forge125to152(file_info[0], version_id, meta[0]);
        }
        
        if(ForgeHandler.forge_version <= 12.4) {
            new Forge11to124(file_info[0], version_id, meta[0]);
        }
